...
## GameDirector와 Blueprint 운영 (2026-09-17)

- 설계·기능·보완·코드 작업 전에 프로젝트 루트 `Blueprint.md`의 방향, 관련 BP ID, 결정 상태, 구현 상태, 완료 기준을 읽는다. 최신 사용자 지시가 문서보다 우선하며 변경된 결정을 문서에 반영한다.
- 사용자가 GameDirector를 부르거나 신규 기능/미완성 보완의 청사진을 논의하면 `.codex/agents/game_director.toml`의 `game_director`를 사용한다. 역할·모델을 사용할 수 없으면 알리고 다른 역할로 조용히 대체하지 않는다. 이미 확정된 일반 구현/오탈자마다 재논의를 강제하지 않는다.
- GameDirector는 gpt-6-astra / medium을 사용하는 읽기 전용 설계 자문이다. 메인이 사용자 질문/답변을 전달하고, 결과 취합 후 메인만 `Blueprint.md`를 작성·갱신한다. 이는 설계 문서 갱신에 대한 상시 요청이며, 설계 논의를 게임 코드 수정으로 확대하라는 뜻은 아니다.
- 기능 제안·사용자 확정·정적 구현 확인·실행 검증을 구분한다. 메인은 실제 코드 작업 착수시 진행중, 종료시 실제 결과/검증 근거/남은 작업을 같은 BP ID에 기록한다. 검증 없는 완료 처리를 금지한다.
- 승인된 범위는 재승인받지 않는다. Blueprint의 '예정'만으로 즉시 착수하지 않으며 현재 사용자 요청과 기존 승인을 함께 확인한다.
- `Blueprint.md`는 제품 청사진, `PLANS.md`와 개별 실행계획은 구현 절차다. 기존 에셋/저장 형식 호환성 및 승인 규칙은 유지한다.
