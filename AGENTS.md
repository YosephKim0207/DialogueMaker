...
## GameDirector와 Blueprint 운영

- 총괄–GameDirector의 단일 작업 흐름은 `Docs/AI/GameDirector.md`를 유일한 운영 기준으로 적용한다. 제품 설계 논의·인계·구현 중 반환·완료 검토 절차를 다른 파일에 중복 정의하지 않는다.
- 사용자가 GameDirector를 부르거나 신규 기능/보완의 미확정 제품 설계를 논의하면 총괄이 `.codex/agents/game_director.toml`의 `game_director` 하나로 연결한다. 역할을 사용할 수 없으면 알리고 다른 역할로 대체하지 않는다.
- 설계·기능·코드 작업 전 `Blueprint.md`의 관련 결정·BP 상태·완료 기준을 확인한다. 총괄만 문서를 저장하고 구현·검증하며, 이미 승인된 범위를 재승인받지 않는다. 설계 문서 갱신에 대한 상시 요청은 개별 기능 구현 승인을 뜻하지 않는다.
