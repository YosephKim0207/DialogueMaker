...
> 사용자와 GameDirector가 함께 발전·보완하는 제품 방향·기능의 청사진. Director가 논의 기록안을 작성하고 총괄이 저장한다.
> 최초 작성: 2026-09-17. 최신 사용자 지시가 이 문서보다 우선한다.
> 청사진 기록은 플러그인 기능 착수 지시와 다르다. 이미 주어진 구현 승인은 반복해서 요청하지 않는다.

## 1. 기록 기준

- 총괄–GameDirector의 작업 흐름과 기록 시점은 `Docs/AI/GameDirector.md`를 따른다. 이 문서는 기능 논의·제품 결정·BP 상태·인계 결과의 기준 기록이다.
- 미확정 후보·대안·보완점·열린 질문도 기록한다. 이미 확정된 기능은 기존 결정 상태를 유지하면서 미확정 세부사항을 구별한다. 총괄은 사용자 발언·기존 결정·근거와 대조해 저장한다. 기능 작업 전 관련 BP ID·결정/구현 상태·완료 기준을 읽는다.
...
- GameDirector가 논의의 진전과 다음 질문을 기록안으로 정리하면 총괄이 이 문서에 반영한다. 사용자 확정 사항과 미확정 제안을 구분한다.
